...
export namespace Shopify {
  ...
  export async function shipping(page: Page): Promise<void> {
    ...
    const el = await page.$(shippingSelector); // I am sure there will be an element to return here
  ...
  export async function creditCard(page: Page, paymentDetails: Profiles.Payment): Promise<void> {
    ...
    await cardNumberFrame!.type(creditCardNumberSelector, paymentDetails.cardNumber);
    await pageC.waitForTimeout(500);
    ...
    await nameFrame!.type(nameOnCardSelector, paymentDetails.name);
    ...
    await expiryFrame!.type(creditCardExpirationDateSelector, paymentDetails.exp);
    ...
    await cvvFrame!.type(creditCardCVVSelector, paymentDetails.cvv);
  }
  ...
  export async function queue(cluster: ClusterRunner, profile: string) {
    ...
    if (configOption === null || configOption.type !== "shopify") {
      ...
      cluster.queue(async ({ page }) => {
        ...
        const httpResponse = await page.waitForNavigation({ timeout: 20000 });
        if (httpResponse) {
          ...
          console.log("Encountered error checking out..");
        }
      });
      await cluster.idle();
    }
